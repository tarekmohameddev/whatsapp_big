                                    ...
                                    <td data-label="{{ translate('Status')}}">
                                        <div class="switch-wrapper checkbox-data">
                                            <input {{ $language->status == \App\Enums\StatusEnum::TRUE->status() ? 'checked' : '' }}
                                                    ...
                                                    data-id="{{ $language->id }}"
                                                    data-column="status"
                                                    data-value="{{ $language->status == \App\Enums\StatusEnum::TRUE->status() 
                                                        ? \App\Enums\StatusEnum::FALSE->status()  
                                        ...
                                        @else
                                            <div class="switch-wrapper checkbox-data">
                                                <input {{ $language->is_default == \App\Enums\StatusEnum::TRUE->status() ? 'checked' : '' }}
                                                        ...
                                                        data-value="{{ $language->is_default ? \App\Enums\StatusEnum::FALSE->status() : \App\Enums\StatusEnum::TRUE->status() }}"
                                                        data-route="{{route('admin.system.language.status.update')}}"
                                    ...
                                    <td data-label={{ translate('Option')}}>
                                        <div class="d-flex align-items-center gap-1">
                                            ...
                                            <button class="icon-btn btn-ghost btn-sm danger-soft circle text-danger delete-language"
                                                    type="button"
                                                    data-language-id="{{ $language->id }}"
                                                    data-url="{{route('admin.system.language.delete', ['id' => $language->id ])}}" 
                                                    data-bs-toggle="modal"
        ...
        <form method="POST" id="deleteLanguageForm">
            ...
            <div class="modal-body">
                ...
                <div class="action-message">
                    <h5>{{ translate("Are you sure to delete this langugae?") }}</h5>
...
<script>
	"use strict";
        ...
        $('.delete-language').on('click', function() {
            ...
            var modal = $('#deleteLanguage');
            modal.find('form[id=deleteLanguageForm]').attr('action', $(this).data('url'));
            modal.modal('show');
        });
