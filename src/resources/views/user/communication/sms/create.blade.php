    ...
    <div class="container-fluid px-0 main-content">
      ...
      <div class="card">
        ...
        <div class="card-body pt-0">
          <form action="{{route('user.communication.sms.store', ['type' => 'sms'])}}" method="POST" enctype="multipart/form-data" id="sms_send">
            @csrf
            ...
            @if($planAccess->type == App\Enums\StatusEnum::FALSE->status())
              <div class="form-element">
                  <div class="row gy-3">
                    <div class="col-xxl-2 col-xl-3">
                        ...
                        <div class="col-12">
                          <div class="form-inner">
                            ...
                            <select class="form-select select2-search" id="chooseMethod" data-placeholder="{{ translate("Choose a sending method") }}" aria-label="chooseMethod" name="method">
                              <option value=""></option>
                              @if(array_key_exists('is_allowed', $planAccess->android) && $planAccess->android['is_allowed'])
                                <option value="{{ \App\Enums\System\Gateway\SmsGatewayTypeEnum::ANDROID->value }}">{{ translate("Android Gateway") }}</option>
                              @endif
                              @if(array_key_exists('is_allowed', $planAccess->sms) && $planAccess->sms['is_allowed'])
      ...
      $('#chooseMethod').change(function() {
        ...
        selectApiGateway.empty();
          if (method == "{{ \App\Enums\System\Gateway\SmsGatewayTypeEnum::ANDROID->value }}") {
              ...
              var planaccess_type = "{{ $planAccess->type == \App\Enums\StatusEnum::FALSE->status() ? 'true' : 'false' }}"
